...
knn_classifier(5, 100, y_new, Y, image_ids, 'modified_sse', E)
```
...
```{r}
find_best_params <- function(M,image_ids, metrics, param_grid, num_folds = 5, D, num_people_to_exclude=1) {
  ...
  for (fold in seq_along(folds)) {
    ...
    E <- pca_results$E
    Y <- sweep(M, 2, mean_obs, FUN = "-") %*% P
    cat("PCA computed and projections done.\n")

    for (n_pc in param_grid$n_pcs){  
      Y_pc <- Y[, 1:n_pc, drop = FALSE]
      E_pc <- E[1:n_pc, drop = FALSE]
...
results_thresholds <- find_best_params(M,image_ids=image_ids, metrics=c('modified_sse', 'simplified_mahalanobis', 'weighted_angle'), 
                                       param_grid=param_grid_thresholds, num_folds = 15, D, num_people_to_exclude=2)
...
best_thresholds_per_fold <- results_thresholds %>%
...
results_final <- find_best_params(Y, image_ids=image_ids, metrics=c('modified_sse', 'simplified_mahalanobis', 'weighted_angle'), 
                                  param_grid=param_grid_others, num_folds = 15, D, num_people_to_exclude=2)
...
# Print the results
print(paste("Best Metric:", best_metric))
print(paste("Final Average k:", final_avg_k))
print(paste("Final Average n_pcs:", final_avg_n_pcs))
