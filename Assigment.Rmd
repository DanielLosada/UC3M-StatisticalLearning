...
for (i in seq_along(E)) {
  cat("Principal component ", i, " explains ",E[i] / sum(E)*100, "% of the variance.\n")
}
```
...
y_new <- project_image(new_image_vector, mean_obs, P)
...
```{r}
create_random_splits_by_image <- function(image_ids, percent_test = 0.2, num_splits = 5, num_people_to_exclude = 1) {
...
splits_by_img <- create_random_splits_by_image(image_ids, num_people_to_exclude=1)
splits_by_img
```
...
```{r}
#Not used
create_random_splits_by_people <- function(image_ids, percent_test = 0.2, num_splits = 5) {
...
#threshold_splits <- create_random_splits_by_people(image_ids)
```


```{r}
find_best_params <- function(Y, image_ids, metrics, param_grid, num_folds = 5, D, num_people_to_exclude=1) {
  folds <- create_random_splits_by_image(image_ids, num_splits=num_folds, num_people_to_exclude=num_people_to_exclude)
  results <- list()
        ...
        for (k in param_grid$k_values) {
          for (threshold in param_grid[[metric]]) {
            ...
            predictions <- sapply(1:nrow(Y_test), function(i) {
              knn_classifier(k, threshold, Y_test[i, ], Y_train, image_ids_train, metric, D_pc)
            })
            ...
            results <- rbind(results, data.frame(Fold = fold, n_pcs = n_pc, Metric = metric, k = k, Threshold = threshold, 
...
param_grid_thresholds <- list(
  ...
  simplified_mahalanobis = seq(-6000, -1500, by = 100),# Threshold
  weighted_angle = seq(-0.07, -0.009, by = 0.005)# Threshold
)

results_thresholds <- find_best_params(Y, image_ids=image_ids, metrics=c('modified_sse', 'simplified_mahalanobis', 'weighted_angle'), 
                                       param_grid=param_grid_thresholds, num_folds = 5, D, num_people_to_exclude=2)
...
```{r}
best_thresholds <- results_thresholds %>%
  group_by(Metric) %>%
  slice_max(Balanced_Accuracy, n = 1, with_ties = FALSE) %>%  # Ensures only one row per metric
  select(Metric, Threshold, Balanced_Accuracy) %>%
  ungroup()  # Remove grouping for a clean dataframe

print(best_thresholds)

best_thresholds_vector <- best_thresholds %>%
  pull(Threshold, name = Metric)
...
param_grid_others <- list(
  ...
  #k_values = c(1),
  modified_sse = c(best_thresholds_vector["modified_sse"]),
  simplified_mahalanobis = c(best_thresholds_vector["simplified_mahalanobis"]),
  weighted_angle = c(best_thresholds_vector["weighted_angle"])
)

results_final <- find_best_params(Y, image_ids=image_ids, metrics=c('modified_sse', 'simplified_mahalanobis', 'weighted_angle'), 
                                  param_grid=param_grid_others, num_folds = 5, D, num_people_to_exclude=2)
...
```{r}
best_per_fold <- results_final %>%
  group_by(Fold) %>%
  slice_max(Balanced_Accuracy, with_ties = FALSE)  # Selects the row with the highest Balanced Accuracy
...
```{r}
find_best_threshold <- function(Y, image_ids, metric, threshold_values, num_folds = 5, D, folds) {
...
find_best_threshold(Y, image_ids, "modified_sse", seq(3e-5, 2.5e-4, by = 5e-6), 5, D, splits_by_img)
```
