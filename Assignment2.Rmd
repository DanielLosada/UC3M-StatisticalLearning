...
# Example call:
segment_image_with_em("Melanoma.jpg", K = 2, tol = 1e-4, rgb = FALSE)

```

```{r}
segment_image_with_em("Composition_A_Mondrian.jpg", K = 2, tol = 10^-6)
```


```{r}
# Example calls to segment different images

#segment_image_with_em("Spaceship.jpg", K = 4, tol = 1e-2)


```

```{r}
segment_image_with_em("Stats.jpg", K = 4, tol = 1e-1)
```
```{r}
segment_image_with_em("Locura3.jpg", K = 5, tol = 1e-4)
```
