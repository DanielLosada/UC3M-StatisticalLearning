...
$$
x\_pc1 = y1 \times p1 + \text{mean\_obs},
$$
where $p1$ is the chosen principal component and $y1$ is the image’s coordinate along this component. Once we have this reconstructed vector, we reshape it into the original image dimensions (height, width, and 3 color channels) to form an image matrix that we can display using `OpenImageR::imageShow()`.
...
As our k-NN classifier relies on computing distances between vectors, we decided to try three different metrics that showed promising results in the paper "Distance measures for PCA-based face recognition." We implemented these metrics—"simplified_mahalanobis", "weighted_angle", and "modified_sse"—to see which one works best for our application.
...
For the `"simplified_mahalanobis"` metric, we first set a constant $\\alpha = 0.25$. The function then computes a weighting vector $z$ as follows:
...
```{r PCA-KNN-THRESHOLD, include=FALSE, cache=TRUE}
...
```{r PCA-KNN-OTHERS, include=FALSE, cache=TRUE}
if(COMPUTE_GRID_SEARCH){
...
## Compare PCA vs Original Image Representation (Find Best Parameters Original Image Representation)
...
```{r NO-PCA THRESHOLD, include=FALSE, cache=TRUE}
if(COMPUTE_GRID_SEARCH){
...
```{r NO-PCA OTHERS, include=FALSE, cache=TRUE}
if(COMPUTE_GRID_SEARCH){
  cat("Finding best parameters...\n")
  best_thresholds_no_pca_results <- readRDS("best_thresholds_no_pca_results.rds")
...
```{r PCA-FDA THRESHOLD, cache=TRUE}
if(COMPUTE_GRID_SEARCH){
  cat("Finding best threshold...\n")
  param_grid_thresholds <- list(
    n_pcs = c(30),  # Fixed n_pcs
    ...
    modified_sse = seq(3e-5, 2.5e-4, by = 5e-6), # Threshold 3e-5
    simplified_mahalanobis = seq(-6000, -1500, by = 100),# Not used
    weighted_angle = seq(-0.07, -0.009, by = 0.005)# Not used
  )

best_thresholds_fda_results <- get_best_params(best_threshold=TRUE, M,image_ids=image_ids, metrics=c('modified_sse', 'simplified_mahalanobis', 'weighted_angle'), param_grid=param_grid_thresholds, num_folds = 15, num_people_to_exclude=2, compute_pca=TRUE, apply_fda = TRUE)
...
best_thresholds_fda_results$best_thresholds_per_fold
best_thresholds_fda_results$average_best_thresholds
...
if(COMPUTE_GRID_SEARCH){
  ...
  if(COMPUTE_GRID_SEARCH){
    ...
    param_grid_others <- list(
      ...
      simplified_mahalanobis = c(best_thresholds_fda["simplified_mahalanobis"]),
      weighted_angle = c(best_thresholds_fda["weighted_angle"])
    )
    
    best_params_fda <- get_best_params(best_threshold=FALSE, M,image_ids=image_ids, metrics=c('modified_sse', 'simplified_mahalanobis', 'weighted_angle'), param_grid=param_grid_others, num_folds = 15, num_people_to_exclude=2, compute_pca=TRUE, apply_fda = TRUE)
...
best_params_fda$best_per_fold
best_params_fda$average_k_npcs_by_metric
